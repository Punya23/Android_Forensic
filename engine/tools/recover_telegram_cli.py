...
def _print_schema(schema: dict) -> None:
    ...
    print(f"  Columns ({schema.get('col_count', 0)}): "
...
def _print_summary(counts: dict) -> None:
    ...
    print(f"  {_CYAN}Recovered — Verified    {_RESET}: {counts.get('recovered_verified', 0)}")
    print(f"  {_YELLOW}Carved — Partial        {_RESET}: {counts.get('carved_partial', 0)}")
    print(f"  {_RED}Deletion Detected       {_RESET}: {counts.get('deletion_detected', 0)}")
...
def _print_messages(messages: list[dict], verbose: bool, limit: int = 50) -> None:
    ...
    for i, msg in enumerate(shown, 1):
        ...
        if verbose:
            ...
            print(f"      {_DIM}carve_method: {msg.get('carve_method', '')}"
...
def main() -> int:
    ...
    parser.add_argument("db_path", metavar="CACHE4_DB",
