...
from triage.parsers.appchat import (
    looks_like_message,
    best_content,
        ...
        [
            {
                ...
                "notable": True,
                "friendly_name": "Calculator Vault",
                "first_install": 1740000000000,
                "granted_permissions": [
...
def _ig_db(path: Path):
    ...
    for i, (u, t) in enumerate(
        [
            ("7", "meet at the docks tonight"),
            ("8", "delete this message now"),  # row _id=2 (will be deleted)
            ("7", "bring the cash please"),
        ]
    ):
        con.execute(
            "INSERT INTO messages(thread_id_published,user_id,text,timestamp) VALUES (?,?,?,?)",
        ...
        json.dumps(
            {
                "messages": [
                    {
                        "sender_name": "Imran",
                        "content": "the deal is on",
                        "timestamp_ms": 1751826000000,
                    }
                ]
            }
        )
...
def test_snapchat_recovers_and_resolves_identity(tmp_path):
    ...
    con.execute(
        "CREATE TABLE conversation_message (_id INTEGER PRIMARY KEY, client_conversation_id TEXT, "
        ...
        json.dumps(
            {
                "Received Saved Chat History": [
                    ...
                    },
                    {
                        "From": "imran_k",
                        "Media Type": "IMAGE",
                        "Created": "2026-01-01 12:01:00 UTC",
                    },
                ],
                "Sent Saved Chat History": [
                    {
                        "From": "me",
                        "Media Type": "TEXT",
                        "Created": "2026-01-01 12:02:00 UTC",
                        "Text": "understood",
                    },
                ],
            }
...
def test_app_finder_discovers_and_classifies(tmp_path):
    ...
    con.execute(
        "CREATE TABLE chat_log (id INTEGER PRIMARY KEY, from_id TEXT, body TEXT, sent_at INTEGER)"
    )
    for i, (f, b) in enumerate(
        [("alice", "secret meeting at noon"), ("bob", "confirmed, usual place")]
    ):
        con.execute(
            "INSERT INTO chat_log(from_id,body,sent_at) VALUES (?,?,?)",
            (f, b, 1751826000 + i),
        )
    con.commit()
    con.close()
...
def test_thread_conversations_resolves_names():
    ...
    convs = thread_conversations(
        msgs, [{"id": "u1", "name": "Imran"}, {"id": "u2", "name": "Rahul"}]
    )
    assert "c1" in convs
