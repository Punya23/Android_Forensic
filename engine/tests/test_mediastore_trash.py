...
def _artifact(source_path: str, **kw) -> ArtifactRecord:
    return ArtifactRecord(
        ...
        md5="x", tier=Tier.TIER0, method="adb pull", extracted_at="",
...
def test_recovered_trashed_file_is_recovered_verified_with_deletion_time():
    ...
    expected = (datetime.fromtimestamp(expires, tz=timezone.utc)
                - timedelta(days=TRASH_WINDOW_DAYS)).date().isoformat()
...
def test_db_only_trashed_row_is_deletion_detected_not_recovered():
    inv = [MediaInventoryItem(media_id=7, kind="video", display_name="ghost.mp4",
                              size_bytes=9999, is_trashed=True)]
...
def test_recovered_and_detected_are_not_conflated():
    ...
    inv = [MediaInventoryItem(media_id=2, kind="video", display_name="ghost.mp4",
                              is_trashed=True)]
...
def test_db_and_filesystem_evidence_merge_into_one_record():
    ...
    inv = [MediaInventoryItem(media_id=1, kind="image", display_name="evidence.jpg",
                              size_bytes=318, is_trashed=True, owner_app="Camera",
                              date_expires=_iso(datetime.fromtimestamp(expires, tz=timezone.utc)))]
    man = [_artifact(f"/sdcard/DCIM/Camera/.trashed-{expires}-evidence.jpg")]
    res = analyze_mediastore_trash(inv, man, now=NOW)

    assert len(res["items"]) == 1, "the same item from both sources must not double-count"
    it = res["items"][0]
...
def test_pending_is_labelled_not_a_deletion():
    inv = [MediaInventoryItem(media_id=5, kind="image", display_name="part.jpg",
                              is_pending=True)]
    res = analyze_mediastore_trash(inv, [], now=NOW)
    it = res["items"][0]
...
def test_untrashed_media_produces_nothing():
    inv = [MediaInventoryItem(media_id=9, kind="image", display_name="normal.jpg",
                              is_trashed=False, is_favorite=True)]
    res = analyze_mediastore_trash(inv, [], now=NOW)
    assert res["items"] == []
