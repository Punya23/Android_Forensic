...
def generate_report(case_dir: str | Path) -> Path:
    ...
    if oem_quirks:
        _QUIRK_LABELS: dict[str, str] = {
            "knox_container":              "Samsung Knox Secure Folder: app-private data inside is separately encrypted; cannot be acquired via ADB.",
            "secure_folder_opaque":        "Samsung Secure Folder mount is not reachable via the ADB shell.",
            "logsprovider_db":             "Samsung call log is in com.sec.android.provider.logsprovider (not the standard AOSP calllog.db).",
            "mi_account_usb_auth":         "Xiaomi/HyperOS: USB Debugging (Security settings) requires a linked Mi Account and an active SIM.",
            "install_via_usb_toggle":      "Xiaomi/HyperOS: 'Install via USB' must be enabled in Developer Options before APK installation.",
            "aggressive_battery_kill":     "MIUI/HyperOS battery saver may kill the Collector APK mid-run. Disable battery optimization.",
            "usb_install_password_prompt": "ColorOS/Realme UI: OS may prompt for lock screen PIN during `adb install`.",
            "aggressive_process_kill":     "ColorOS/Realme UI may kill background processes. Keep the Collector in the foreground.",
            "auto_launch_deny":            "ColorOS/Realme UI may block the Collector from auto-starting on ADB trigger.",
            "pm_grant_blocked":            "OnePlus/OxygenOS: `pm grant` is blocked; runtime permission dialog used instead.",
            "usb_debug_timeout":           "Honor MagicOS: USB debugging auth may time out. Re-authorize in Developer Options if needed.",
            "magic_link_interference":     "Honor MagicOS: Magic Link cross-device feature may interfere with ADB sessions.",
            "harmonyos_check_required":    "Huawei HarmonyOS: Only AOSP-based versions (≤3.x) support ADB extraction.",
            "adb_may_be_absent":           "Huawei HarmonyOS NEXT: Standard Android ADB may be absent on this device.",
            "google_services_absent":      "Huawei: No GMS — Google account / Play Store artifacts will not be present.",
        }
        quirk_items = "".join(
