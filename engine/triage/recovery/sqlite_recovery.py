...
def _try_carve_cell(buf: bytes, off: int, expected_cols: Optional[int]) -> \
    ...
    while off < end:
        ...
        if carved:
            ...
            if rec_key not in seen_records or provenance_kind == "freeblock":
                ...
                rows.append(CarvedRow(
                    values=values, confidence=conf, source_file=source_file,
...
def read_live_rows(db_path: str | Path, table: str,
                   limit: int = 100000) -> list[CarvedRow]:
    ...
    try:
        ...
        for r in cur.fetchall():
            rows.append(CarvedRow(
                values=list(r[1:]), confidence=Confidence.LIVE,
                source_file=db_path.name, provenance="live query",
                rowid=r[0],
            ))
        con.close()
...
def detect_rowid_gaps(db_path: str | Path, table: str) -> list[dict[str, Any]]:
    ...
    try:
        con = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True)
        rowids = [r[0] for r in con.execute(f"SELECT rowid FROM '{table}' ORDER BY rowid")]
        con.close()
    except sqlite3.Error:
        return []
...
def recover_all(db_path: str | Path) -> dict[str, Any]:
    ...
    for tname in schema:
        ...
        try:
            con = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True)
            live_counts[tname] = con.execute(f"SELECT COUNT(*) FROM '{tname}'").fetchone()[0]
            con.close()
        except sqlite3.Error:
            live_counts[tname] = -1
    return {
