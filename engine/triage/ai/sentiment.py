...
def analyze_sentiment_over_time(messages: List[Dict]) -> List[Dict[str, Any]]:
    ...
    sorted_msgs = sorted(
        [m for m in messages if m.get("timestamp")], key=lambda x: x["timestamp"]
    )
...
def generate_sentiment_report(sentiments: List[Dict]) -> str:
    ...
    if spikes:
        ...
        for spike in spikes:
            ...
            html_out.append(
                f"<li><span style='color: {color}'>[{spike['type'].upper()}]</span> "
            )
            html_out.append(
                f"{spike['timestamp']} (Score: {spike['score']:.2f}): {html.escape(spike['snippet'])}...</li>"
            )
        html_out.append("</ul>")
    else:
