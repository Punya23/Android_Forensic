...
CATEGORIES = {
    "threat": re.compile(r'\b(kill|attack|bomb|weapon|shoot|destroy|beat)\b', re.IGNORECASE),
    "fraud": re.compile(r'\b(otp|upi|transfer|account|scam|urgent|money|kyc)\b', re.IGNORECASE),
    "harassment": re.compile(r'\b(bitch|whore|slut|abuse|harass)\b', re.IGNORECASE),
    "financial": re.compile(r'\b(bank|rupees|rs|inr|salary|invest|tax)\b', re.IGNORECASE),
    "location": re.compile(r'\b(meet|address|street|road|city|gps|here)\b', re.IGNORECASE),
    "media": re.compile(r'\b(photo|video|image|picture|selfie|recording)\b', re.IGNORECASE)
}
...
def get_evidence_importance_score(evidence: Dict) -> int:
    ...
    if cat in ["threat", "fraud"]: score += 40
    elif cat in ["harassment", "financial"]: score += 30
    elif cat in ["location", "media"]: score += 20
    else: score += 10
    
    # Confidence weight (max 30)
    conf_str = str(evidence.get("confidence", "live")).lower()
    if conf_str == "live": score += 30
    elif conf_str == "recovered": score += 20
    elif conf_str == "carved": score += 10
    
    # Source weight (max 20)
    source = str(evidence.get("source_file", "")).lower()
    if "whatsapp" in source or "telegram" in source: score += 20
    elif "sms" in source or "mms" in source: score += 15
    else: score += 10
...
def prioritize_evidence(evidence_list: List[Dict]) -> List[Dict]:
    ...
    for ev in evidence_list:
        ...
        if score >= 80: new_ev["priority_tier"] = "critical"
        elif score >= 60: new_ev["priority_tier"] = "high"
        elif score >= 40: new_ev["priority_tier"] = "medium"
        elif score >= 20: new_ev["priority_tier"] = "low"
        else: new_ev["priority_tier"] = "info"
...
def generate_classification_report(classified: List[Dict]) -> str:
    ...
    html_out = ["<div class='ai-classification-report'>", "<h2>AI Evidence Classification</h2>"]
    ...
    html_out.append("<table><tr><th>Tier</th><th>Category</th><th>Score</th><th>Snippet</th></tr>")
    ...
    for ev in classified[:10]:
        ...
        color = "black"
        if tier == "CRITICAL": color = "red"
        elif tier == "HIGH": color = "orange"
