...
def get_investigation_suggestions(evidence_list: List[Dict]) -> List[Dict[str, Any]]:
    ...
    for ev in evidence_list:
        ...
        if cat == "threat":
            has_threats = True
        if conf == "deletion_detected" or conf == "carved":
            has_deleted = True
    ...
    if not suggestions:
        suggestions.append(
            {"suggestion": "Proceed with standard triage reporting.", "priority": "LOW"}
        )
...
def predict_case_outcome(evidence_list: List[Dict]) -> Dict[str, Any]:
    ...
    high_value_count = sum(
        1 for e in evidence_list if e.get("importance_score", 0) >= 60
    )
    ...
    return {"prediction": prediction, "confidence": conf, "factors": factors}


def generate_assistant_report(assistant_data: Dict) -> str:
    """Generate HTML assistant report."""
    html_out = [
        "<div class='ai-assistant-report'>",
    ...
    for r in recs:
        html_out.append(
            f"<li><strong>[{r.get('priority', 'INFO')}]</strong>: {html.escape(r.get('suggestion', ''))}</li>"
        )
    html_out.append("</ul>")
    ...
    if pred:
        html_out.append(
            f"<h3>Case Outcome Prediction: {pred.get('prediction', 'Unknown')}</h3>"
        )
