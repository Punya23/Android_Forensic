...
try:
    ...
    HAS_ML = False
    logging.warning(
        "scikit-learn or numpy not installed. Falling back to basic clustering."
    )
...
def find_similar_evidence(
    target: Dict, evidence_list: List[Dict], top_k: int = 10
) -> List[Dict]:
...
def generate_clustering_report(clusters: List[Dict]) -> str:
    ...
    html_out = [
        "<div class='ai-clustering-report'>",
        "<h2>Evidence Clustering Analysis</h2>",
    ]
    ...
    for cluster in clusters:
        ...
        html_out.append(
            f"<div class='cluster-card'><h3>Cluster {cid} (Size: {c_stat.get('size')})</h3>"
        )
