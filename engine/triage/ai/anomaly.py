...
try:
    ...
    HAS_ML = False
    logging.warning(
        "scikit-learn or numpy not installed. Falling back to basic anomaly detection."
    )
...
def generate_anomaly_report(anomalies: List[Dict]) -> str:
    ...
    html_out.append(
        "<table><tr><th>Anomaly Type</th><th>Score</th><th>Reason</th><th>Snippet</th></tr>"
    )
    ...
    for a in anomalies:
        ...
        html_out.append(
            f"<tr><td><strong>{html.escape(cls_info['type'].upper())}</strong></td>"
        )
