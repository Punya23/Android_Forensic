...
def assign_topics_to_evidence(
    evidence_list: List[Dict], num_topics: int = 5
) -> List[Dict]:
...
def generate_topic_report(topics: List[Dict], evidence: List[Dict]) -> str:
    ...
    if not topics:
        html_out.append(
            "<p>Insufficient data or missing dependencies for topic extraction.</p></div>"
        )
        return "\n".join(html_out)
    ...
    for topic in topics:
        ...
        if samples:
            ...
            for ev in samples[:3]:  # Show top 3
                snippet = (
                    html.escape(str(ev.get("body", ev.get("name", "")))[:60]) + "..."
                )
