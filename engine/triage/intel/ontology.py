...
ARTIFACTS: dict[str, dict] = {
    ...
    "financial":   {"label": "Financial / payment traces", "cost": "cheap", "tier": "tier0"},
...
CRIME_ONTOLOGY: dict[str, CrimeProfile] = {
    "murder": CrimeProfile(
        key="murder", label="Homicide / Murder",
        synonyms=["murder", "murderer", "homicide", "killed", "kill", "stabbed",
                  "shot dead", "manslaughter", "death of", "dead body", "assassinat"],
        priorities={
            "call_logs": "high", "sms": "high", "locations": "high",
            "whatsapp": "high", "telegram": "high", "deleted": "high",
            "contacts": "high", "media": "medium", "browser": "medium",
            "calendar": "medium", "apps": "low", "usage": "low",
        },
        keywords=[r"\b(kill|murder|threat|revenge|weapon|knife|gun|blood|body|hide|dispose)\b",
                  r"\b(alibi|last\s?seen|met|midnight|silence|witness)\b"],
        rationale=("Homicide turns on contact/comms around the time of death, movement "
                   "(GPS) and deleted messages; bulk media matters only if scene/weapon "
                   "photos are suspected."),
    ),
    "drug_trafficking": CrimeProfile(
        key="drug_trafficking", label="Drugs / Narcotics",
        synonyms=["drug", "drugs", "narcotic", "ndps", "cocaine", "heroin", "mdma",
                  "ganja", "peddling", "trafficking", "supplier", "consignment"],
        priorities={
            "whatsapp": "high", "telegram": "high", "sms": "high", "call_logs": "high",
            "financial": "high", "contacts": "high", "locations": "high",
            "deleted": "high", "media": "medium", "apps": "medium", "browser": "low",
            "calendar": "low", "usage": "low",
        },
        keywords=[r"\b(maal|consignment|delivery|drop|package|supply|stuff|score|"
                  r"weed|coke|charas|ganja|mdma|pills|order)\b",
                  r"\b(payment|cash|upi|transfer|hawala|crypto|bitcoin|usdt|wallet)\b"],
        rationale=("Narcotics networks live in encrypted chats + payment traces + "
                   "delivery locations; supplier/buyer contact graph is central."),
    ),
    "financial_fraud": CrimeProfile(
        key="financial_fraud", label="Financial Fraud / Cheating",
        synonyms=["fraud", "cheat", "cheating", "scam", "ponzi", "phishing", "forgery",
                  "embezzle", "money laundering", "fake investment", "loan fraud"],
        priorities={
            "financial": "high", "sms": "high", "whatsapp": "high", "browser": "high",
            "call_logs": "high", "accounts": "high", "contacts": "high",
            "deleted": "high", "apps": "medium", "telegram": "medium",
            "media": "low", "locations": "low", "calendar": "low", "usage": "low",
        },
        keywords=[r"\b(otp|account\s?no|ifsc|upi|transfer|payment|invoice|refund|"
                  r"investment|returns|deposit|withdraw|kyc|loan|emi)\b",
                  r"\b(fake|forged|fraud|scam|dupe|launder|shell\s?company)\b"],
        rationale=("Fraud is a paper trail: SMS bank alerts, payment apps, phishing "
                   "links in browser/comms, and the victim/mule contact list."),
    ),
    "terrorism": CrimeProfile(
        key="terrorism", label="Terrorism / Unlawful Activity",
        synonyms=["terror", "terrorism", "uapa", "bomb", "blast", "ied", "radical",
                  "extremist", "sleeper cell", "attack plot", "jihad"],
        priorities={
            "whatsapp": "high", "telegram": "high", "locations": "high",
            "media": "high", "browser": "high", "deleted": "high", "contacts": "high",
            "call_logs": "high", "sms": "high", "financial": "high", "apps": "medium",
            "accounts": "medium", "calendar": "medium", "usage": "low",
        },
        keywords=[r"\b(bomb|blast|ied|explosive|weapon|attack|target|martyr|"
                  r"operation|handler|training)\b",
                  r"\b(meet|location|coordinates|map|recce|safe\s?house)\b"],
        rationale=("Terror cases are all-source: encrypted comms, propaganda media, "
                   "recce locations, funding, and the handler/cell contact web."),
    ),
    "kidnapping": CrimeProfile(
        key="kidnapping", label="Kidnapping / Abduction",
        synonyms=["kidnap", "kidnapping", "abduct", "abduction", "ransom", "hostage",
                  "missing person", "held captive"],
        priorities={
            "call_logs": "high", "locations": "high", "sms": "high", "whatsapp": "high",
            "contacts": "high", "deleted": "high", "telegram": "medium",
            "financial": "medium", "media": "medium", "browser": "low",
            "calendar": "low", "apps": "low", "usage": "low",
        },
        keywords=[r"\b(ransom|money|drop|location|release|hostage|demand|pay|"
                  r"don'?t\s?call\s?police|deadline)\b"],
        rationale=("Kidnapping hinges on ransom contact, movement of victim/suspect, "
                   "and the call/message trail of demands."),
    ),
    "sexual_offence": CrimeProfile(
        key="sexual_offence", label="Sexual Offence / POCSO",
        synonyms=["rape", "sexual", "molest", "pocso", "assault", "harassment",
                  "obscene", "csam", "child abuse", "stalking"],
        priorities={
            "media": "high", "whatsapp": "high", "instagram": "high", "snapchat": "high",
            "sms": "high", "contacts": "high", "deleted": "high", "call_logs": "medium",
            "browser": "medium", "locations": "medium", "apps": "medium",
            "accounts": "low", "calendar": "low", "usage": "low",
        },
        keywords=[r"\b(photo|video|pic|nude|obscene|share|secret|delete|don'?t\s?tell)\b"],
        rationale=("These cases turn on media evidence and the private-chat apps "
                   "(WhatsApp/IG/Snap) between accused and victim; deleted media is key."),
    ),
    "cybercrime": CrimeProfile(
        key="cybercrime", label="Cybercrime / Hacking",
        synonyms=["cyber", "hacking", "hacked", "malware", "ransomware", "data breach",
                  "identity theft", "sextortion", "darkweb", "dark web"],
        priorities={
            "browser": "high", "apps": "high", "accounts": "high", "sms": "high",
            "financial": "high", "whatsapp": "medium", "telegram": "high",
            "deleted": "high", "contacts": "medium", "media": "medium",
            "call_logs": "low", "locations": "low", "calendar": "low", "usage": "medium",
        },
        keywords=[r"\b(login|password|credential|otp|bitcoin|wallet|ransom|breach|"
                  r"leak|dump|exploit|phish|vpn|tor|darkweb)\b"],
        rationale=("Cyber cases favour browser/app/account artifacts, crypto payment "
                   "traces, and Telegram/dark-web channels over voice calls."),
    ),
    "harassment": CrimeProfile(
        key="harassment", label="Harassment / Stalking / Threats",
        synonyms=["harass", "harassment", "stalk", "stalking", "threat", "abuse",
                  "blackmail", "defamation", "trolling", "cyberbullying"],
        priorities={
            "whatsapp": "high", "instagram": "high", "snapchat": "high", "sms": "high",
            "call_logs": "high", "contacts": "high", "deleted": "high",
            "media": "medium", "browser": "medium", "locations": "low",
            "accounts": "low", "apps": "low", "calendar": "low", "usage": "low",
        },
        keywords=[r"\b(threat|abuse|blackmail|leak|expose|kill\s?you|ruin|screenshot|"
                  r"stalk|follow|watching)\b"],
        rationale=("Harassment is a messaging case: the offending WhatsApp/IG/SMS "
                   "threads and repeated call attempts are the evidence."),
    ),
    "theft": CrimeProfile(
        key="theft", label="Theft / Robbery / Burglary",
        synonyms=["theft", "robbery", "burglary", "steal", "stolen", "dacoity",
                  "snatching", "loot", "break-in"],
        priorities={
            "locations": "high", "call_logs": "high", "sms": "medium", "contacts": "high",
            "whatsapp": "medium", "deleted": "medium", "media": "medium",
            "financial": "medium", "browser": "low", "apps": "low",
            "calendar": "low", "accounts": "low", "usage": "low",
        },
        keywords=[r"\b(sell|buyer|price|jewel|gold|phone|scrap|dispose|fence|loot)\b"],
        rationale=("Property crime favours movement (GPS), co-accused contact, and any "
                   "resale/disposal chatter."),
    ),
    "missing_person": CrimeProfile(
        key="missing_person", label="Missing Person",
        synonyms=["missing", "untraceable", "ran away", "runaway", "disappeared",
                  "whereabouts unknown", "last seen"],
        priorities={
            "locations": "high", "call_logs": "high", "contacts": "high", "sms": "high",
            "whatsapp": "high", "instagram": "medium", "browser": "medium",
            "media": "medium", "calendar": "medium", "deleted": "medium",
            "financial": "low", "apps": "low", "accounts": "low", "usage": "low",
        },
        keywords=[r"\b(leave|leaving|run\s?away|last\s?seen|going\s?to|station|bus|"
                  r"train|ticket|meet|address)\b"],
        rationale=("Locating a person prioritises last-known movement, recent contacts, "
    ...
    "general": CrimeProfile(
        ...
        keywords=[],
        rationale=("No specific crime type detected — running a balanced full triage. "
