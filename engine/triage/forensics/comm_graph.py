...
def build_communication_graph(calls: List[Dict], sms: List[Dict]) -> Dict:
    ...
    graph = {
        "nodes": {},
        "edges": collections.defaultdict(lambda: {"count": 0, "types": set(), "timestamps": []})
    }
    ...
    for call in calls:
        ...
        if num not in graph["nodes"]:
            graph["nodes"][num] = {"id": num, "label": call.get("name", num), "degree": 0}
    ...
    output = {
        ...
        "edges": [
            {"source": k[0], "target": k[1], "weight": v["count"], "types": list(v["types"]), "timestamps": v["timestamps"]}
...
def generate_graph_report(graph: Dict) -> str:
    ...
    if anomalies:
        html_out.append("<ul>")
        for a in anomalies:
             html_out.append(f"<li><strong>{a['severity']}</strong>: {a['source']} <-> {a['target']} - {a['reason']}</li>")
        html_out.append("</ul>")
    else:
