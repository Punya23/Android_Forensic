...
FRAUD_PATTERNS = re.compile(r'\b(otp|upi|transfer|account|urgent|money|kyc|block|credit|debit)\b', re.IGNORECASE)
THREAT_PATTERNS = re.compile(r'\b(kill|attack|bomb|weapon|threat|destroy|beat|shoot)\b', re.IGNORECASE)
HARASS_PATTERNS = re.compile(r'\b(bitch|whore|slut|bastard|abuse)\b', re.IGNORECASE) # Simplified list
...
def generate_legal_report(recommendations: List[Dict]) -> str:
    ...
    if not recommendations:
        ...
        html_out.append("<table><tr><th>Priority</th><th>Action</th><th>Statute</th><th>Evidence Snippets</th></tr>")
        for rec in recommendations:
            ev_count = len(rec.get("evidence", []))
            html_out.append(f"<tr><td><strong>{html.escape(rec.get('priority', ''))}</strong></td>")
