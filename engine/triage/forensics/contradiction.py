...
def check_message_vs_location(
    messages: List[Dict], locations: List[Dict]
) -> List[Dict]:
    ...
    loc_points = [
        ...
        if loc.get("timestamp") and loc.get("latitude") and loc.get("longitude")
    ]
...
def check_location_vs_timeline(
    locations: List[Dict], timeline: List[Dict]
) -> List[Dict]:
...
def detect_contradictions(
    messages: List[Dict], calls: List[Dict], locations: List[Dict], timeline: List[Dict]
) -> List[Dict]:
...
def generate_contradiction_report(contradictions: List[Dict]) -> str:
    ...
    if not contradictions:
        ...
        html_out.append(
            "<table><tr><th>Severity</th><th>Timestamp</th><th>Evidence</th><th>Recommendation</th></tr>"
        )
