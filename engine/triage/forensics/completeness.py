...
def check_artifact_completeness(artifact_type: str, case_dir: Path) -> Dict:
    ...
    try:
        ...
        return {"status": "error", "count": 0, "reason": "Failed to load case or manifest."}
    
    count = sum(1 for item in manifest if item.category == artifact_type)
    
    # Very basic heuristics for demo purposes
    if count > 0:
        return {"status": "complete", "count": count, "reason": f"Found {count} artifacts of type {artifact_type}."}
    else:
        return {"status": "skipped", "count": count, "reason": f"No {artifact_type} artifacts found."}
...
def check_tier_completeness(tier: str, case_dir: Path) -> Dict:
    ...
    if count > 0:
        return {"status": "complete", "reason": f"Tier {tier} extraction yielded {count} items."}
    else:
...
def update_completeness_status(case_dir: Path, artifact: str, status: str, reason: str) -> None:
...
def generate_completeness_report(checklist: Dict) -> str:
    ...
    html_out = ["<div class='completeness-report'>", "<h2>Acquisition Completeness</h2>"]
    
    html_out.append("<h3>Artifact Categories</h3>")
    html_out.append("<table><tr><th>Category</th><th>Status</th><th>Count</th><th>Reason</th></tr>")
    ...
    for cat, data in checklist.get("artifacts", {}).items():
        ...
        html_out.append(f"<td style='color: {status_color};'>{html.escape(data.get('status', ''))}</td>")
