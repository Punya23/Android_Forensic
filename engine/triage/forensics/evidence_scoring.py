...
def calculate_timestamp_score(timestamp: str) -> int:
    ...
    try:
        ...
        if delta_days <= 7: return 30
        if delta_days <= 30: return 20
        if delta_days <= 180: return 10
        if delta_days <= 365: return 5
        return 2
    except Exception:
        return 0
...
def generate_scoring_report(evidence_list: List[Dict]) -> str:
    ...
    if not evidence_list:
        ...
        html_out.append("<table><tr><th>Item Details</th><th>Confidence</th><th>Source(s)</th><th>Timestamp</th><th>Correlations</th><th>Total Score</th></tr>")
        ...
        for score, item in scored_items:
            summary = html.escape(str(item.get("body", item.get("name", item.get("label", "Unknown Item"))))[:50])
            ...
            # Sub-scores
            c_score = calculate_confidence_score(item.get("confidence", Confidence.LIVE))
