...
| Item | State |
|---|---|
| `engine/security/`, `engine/analytics/`, `engine/integration/`, `engine/advanced_forensics/` | **Removed (2026-09).** Were present on disk with zero import references anywhere in the codebase. Audited file by file: every one was a **fabrication stub**, not merely unfinished — `security/audit.py`'s `verify_integrity()` always returned `True` regardless of file content; `security/hsm_integration.py`'s `sign_evidence()` returned the literal string `"hsm_digital_signature_placeholder"` for any input; `security/legal/section_65b.py` certified against the Indian Evidence Act 1872 s.65B, repealed and replaced by BSA 2023 s.63; `analytics/vision/face_recognition.py` and `object_detection.py` returned an identical hardcoded detection for every image regardless of content; `analytics/vision/ocr_tamper.py`'s tamper detector always reported "low probability"; `advanced_forensics/filesystem/file_recovery.py` was exactly the raw-image slack-space carver this project's own "do NOT build" list forbids; `advanced_forensics/memory/volatility_runner.py` targeted full physical-memory dumps this tool has no acquisition path to obtain, with field values lifted from a Windows kernel process template. Deleted rather than wired — see `docs/CAPABILITIES.md` "New in v0.7" for what was built instead where a real, honest version of the underlying idea existed. |
| `engine/triage/notifications/` | Real, working code (SMTP/Twilio/Slack/Teams clients). Wired in v0.7 (opt-in, off by default) to fire on acquisition completion; see `docs/CAPABILITIES.md`. |
| `engine/triage/parsers/whatsapp_batch.py` | **Wired (2026-09).** A real, unit-tested multi-export batch parser (`_chat.txt`/`.zip` exports, live `msgstore.db`, `.crypt15/14/12` encrypted backups) had zero call sites. Now reachable via `POST /api/case/<id>/import/whatsapp` (multi-file upload + optional decryption key), merging into the same `messages` dataset a live acquisition writes — no separate dashboard view needed. UI: a batch-upload control on the Messages view. |
| `engine/triage/pipeline.py` perf/hash wrapper functions (`_initialize_optimizations`, `_get_optimal_file_order`, `_track_performance`, `_generate_performance_summary`, `_initialize_hashing`, `_process_hash`, `_verify_hash`, `_update_hash_progress`) | **Removed (2026-09).** Zero call sites, and the hashing ones duplicated work already done correctly elsewhere (per-file hashing happens inline at pull time in `custody.py`). Backing modules `forensics/prefetch.py`, `profile_optimizer.py`, `performance_dashboard.py`, `continuous_hash.py`, `hash_alerts.py` deleted alongside them — each was reachable only from these wrappers, with no test coverage. |
| `forensics/integrity_report.py` (`generate_integrity_report`) and `forensics/auto_verify.py` (`auto_verify_on_open`) | **Wired (2026-09), not deleted** — both are real and already built on the fixed hash-verification schema (P0-1), just had no call site of their own (only reachable via the now-removed `_generate_hash_report`/`_auto_verify_on_complete` pipeline wrappers, which called them from the wrong lifecycle point). Moved to where they actually belong: `generate_integrity_report` now runs once at the end of every acquisition, writing `reports/detailed_hash_integrity.html`; `auto_verify_on_open` now runs on case-open (`GET /api/case/<id>` → `summary.hash_verification`), which is what its own docstring says it's for. Dashboard: Overview's "Integrity & speed" card. |
| `deploy/docker-compose.yml` | **Removed (2026-09).** Referenced a missing `Dockerfile.gateway` and described an `api_gateway`/`graphql_server`/`webhook_worker`+Redis architecture that never existed elsewhere in the project. |
| Electron packaging (`electron:build`) | **Wired (2026-09).** `app/package.json` now has an `electron-builder` `build` config (`extraResources` maps `engine/dist/triage-engine` → packaged `resources/engine/`) and `electron-builder` is a declared devDependency; `npm run build:engine` (new) runs PyInstaller against `engine/snagr.spec` first, and `electron:build` runs it automatically. |
| APK release signing | **Wired (2026-09).** `apk/app/build.gradle` reads `apk/keystore.properties` (git-ignored; see `apk/keystore.properties.example`) when present; `assembleRelease` falls back to the debug key when it isn't, so the output is always signed — never the previous silent unsigned APK. |
| CI/CD | **Added (2026-09).** `.github/workflows/ci.yml` — engine `pytest`, dashboard typecheck+build, and a debug APK build on every push/PR. |
| License | **Added (2026-09).** MIT, root `LICENSE` file. |
...
| `engine/triage/parsers/whatsapp_advanced.py`, `telegram_advanced.py`, `engine/triage/forensics/{financial,legal,location_enhanced}.py` ("Phase 2") | The exception on the "computed" side of this table, not the "wired" side: real code, not a stub — every function computes from real input (live SQLite queries against WhatsApp/Telegram schemas, regex-extracted UPI/bank/txn IDs, Haversine-based visit clustering, keyword-matched statute lookup), and 18 tests (`tests/test_phase2_modules.py`) assert exact computed values, not just "doesn't throw". Genuinely called from `run_acquisition()` (`pipeline.py:2367-2592`) and writes 11 real derived datasets — `whatsapp_reactions`, `whatsapp_admins`, `whatsapp_call_analysis`, `telegram_bots`, `telegram_group_stats`, `upi_transactions`, `money_trail`, `matched_statutes`, `fir_draft`, `expert_report`, `visit_durations` — on every acquisition. But none of the 11 names are in `server.py`'s derived-dataset whitelist, so `GET /api/case/<id>/derived/<name>` 404s on all of them, and nothing in `app/src` or the report generator reads them back. The analysis runs and is saved to disk every single case; an examiner using the tool never sees it. Fix is small — add the 11 names to the whitelist and give them a dashboard panel — which is exactly why this is listed as a gap rather than wired scaffolding: unlike the rows below, the hard part is already done. |
| `engine/triage/case_management/`, `engine/triage/collaboration/`, `engine/triage/intelligence/` ("Phase 5") | Real code — reads/writes real per-case JSON, computes real set-intersection cross-referencing and real Jaccard-weighted pattern similarity, 22 tests pass — but **zero import references outside its own test file** anywhere in the repo, the same unwired pattern as the fabrication stubs above, without being fabricated. Two of `sharing.py`'s functions are honestly-labelled-but-static placeholders that ignore their input regardless (`_extract_victim_profile`, `_extract_temporal_pattern`, both commented `# In production, would...`). Its headline "cross-case linking by phone/UPI/email" duplicates a feature that already ships, wired, under a different module — `registry.py::find_linked_cases` + `GET /api/case/<id>/linked-cases`, live since v0.7 — so this is a second, dead implementation of something the tool already does correctly. `jurisdiction.py::export_to_ncrb()` also reads a `meta.get("ipc_sections", [])` field; the IPC was repealed and replaced by the BNS effective July 2024 — the same class of stale-citation mistake this file already caught and fixed once, for the Evidence Act 1872 → BSA 2023 s.63. Not wired, and the cross-case-linking half specifically should not be — the real version already exists. |
| `engine/triage/intel/{prioritization,summarization,social_network}.py`, `engine/triage/forensics/{multilingual_advanced,behavioral_analysis}.py` ("AI Enhancement Modules") | Real code — weighted heuristic evidence scoring (not literal ML — no model is trained or fit, despite the name), hand-rolled graph centrality (betweenness/closeness/eigenvector, no library), burst/timing-pattern detection, a genuine 54-term Hindi/Hinglish slang map with 31 emoji and 7 script patterns. 17 tests (`tests/test_ai_modules.py`) assert real computed output. Importable via `intel/__init__.py` / `forensics/__init__.py` and exercised only by their own test file — zero call sites in `pipeline.py`, `server.py`, or `app/src`. Same unwired pattern as the row above. |
