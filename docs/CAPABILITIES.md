...
|---|---|---|
| **Per-dataset capability states** (`triage/capabilities.py`, `GET /api/case/:id/capabilities`) | Engine + every view | Resolves each dataset to exactly one of `populated` / `empty` / `not_collected` / `inaccessible` / `planned`, with the reason, the acquisition flag that gates it, and `flag_actionable` — the boolean saying whether re-running with that flag on would actually change this outcome. The flag is named in every payload so the reason can quote it; `flag_actionable` is what decides whether it may be *offered as the fix*, and the badge legend below turns on it. It is settled in the engine, beside the reason text, so the dashboard never re-derives it from the state string and gets it subtly different |
...
| **Unconditionally-written datasets need corroboration** | Engine | `collector_wifi`/`collector_bluetooth`/`search_history`/`browser` are written on every run; an empty one is only reported as "checked" when something proves the stage executed — a sibling dataset (the Collector's run manifest) or the file's own record of having run (ALEAPP's `available`, the trash analysis' `summary`). `browser` is in that list because the Tier-0 read only sees a History file already sitting in shared storage; it and the `search_history` derived from it now report the same thing about the same source instead of contradicting each other four rows apart in the sidebar |
| **Fixed-shape envelopes are not "collected"** (`content_paths`) | Engine | Several stages persist a dict whose keys exist whether or not anything was collected — `aleapp` is `{available, artifacts, report_dir, error}` before ALEAPP is looked for, `discovered_chats` is `{tables: [], messages: []}`, `graph` always carries the owner hub node. A length test calls all of those populated, so the badge read "Collected" for a stage that never started — with no banner and no empty-state override, the one failure mode that looks healthy on screen. Emptiness is a property of the capability now: it names where its content lives, and counters count (`meta.total_messages`, `stats.participants`) |
| **Sidebar state badges** (`opt-in` / `not run` / `n/a` / `soon` / `0`) | Dashboard | The gaps in a run are visible before clicking into forty views, and each badge carries a different promise: `opt-in` — the named flag was off and re-running with it on **will** collect this (the engine asserts it in `flag_actionable`); `not run` — a real gap the flag will not close, so the reason names what does (a case brief, an account-data export import); `n/a` — this handset could not produce it at all and re-running changes nothing; `soon` — not built, no date; `0` — the stage ran and the source was empty, which is a finding about the device |
| **Root outranks the flag, except where a root-free route exists** | Engine | A Tier-2 stage that was *both* switched off and running on an unrooted handset badges `n/a`, not `opt-in`: offering a toggle the engine already knows changes nothing costs a second acquisition and a second set of device-state changes on evidence. The exception is a dataset with its own non-root route (`root_only=False` — Instagram/Snapchat/Telegram conversations, fillable from an imported account-data export), which stays a closable gap naming that route — including when the stage recorded its own pull failure, which settles *why* the pull failed and not whether the gap can be closed |
| **Root availability survives an aborted run** | Engine | `derived/device_state.json` is written at 95%; a crashed, cancelled or in-flight case falls back to `case.json` → `pre_state` and `derived/encryption_state.json`, both written in the first 4%. Unknown stays `null` — it is rendered as neither "rooted" nor "not rooted" |
