...
| **Unconditionally-written datasets need corroboration** | Engine | `collector_wifi`/`collector_bluetooth`/`search_history` are written on every run; an empty one is only reported as "checked" when a sibling (the Collector's run manifest, the parsed browser history) proves the stage executed |
